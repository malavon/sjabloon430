...
class PinsetEventer : public FormEventHandler {
  public:
	PinsetEventer(Window &win, EventEmittingForm<PinsetEventer> &form, const vector<string> &pkgs, unordered_map<string, string> &signals, int row,
				  int sgnCol) : window(win), form(form), packages(pkgs), signalAndDescMap(signals), row(row), signalColumn(sgnCol) {
