...
// helper functions
int scrollToAccomodate(Window &win, int rows, int &currentRow) {
	int rowsToScroll = std::max<int>(0, currentRow + rows - win.maxRows());
	if ( rowsToScroll > 0 ) {
...
void drawPinSetEditingWindow(Window &win, PinSetView &vw) {
	...
	for ( PinSetView::iterator it = vw.begin(); it != vw.end() && (roomToDisplayMore || !selectionReached); it++, idx++ ) {
		...
		if ( idx == static_cast<int>(vw.selIdx) ) {
...
void drawSetConfigWindow(BorderedWindow &win, const vector<Configset> &cfs) {
	...
	unsigned int reqRows = 0, reqAllMdl = 0, reqAllPkg = 0, reqDefault = 0, reqOther = 0;
	for ( const Configset &cs : cfs ) {
		unsigned int reqMdl = cs.toModels().size() /* one line per model */ + 1 /* header */;
		unsigned int reqPkg = cs.toPkgs().size() / 2 /* packages are max 5 wide, 2 pkgs/line */
	...
	bool canDisplayAll = reqRows <= maxRows;
	bool canDisplayAllIfNoHeaders = !canDisplayAll && (reqRows - cfs.size() * 2) <= maxRows;
	bool canDisplayDefIfNoPkgs = reqRows - reqAllPkg <= maxRows;
	bool canDisplayAllButDef = reqOther <= maxRows;
	...
	bool displayMdl = reqAllMdl <= maxRows;
...
void loopPinsetEditing(Window &win, Window &hot, BorderedWindow &config, ui::PinSetView &vw) {
	...
	do {
		switch ( tempChar ) {
			case KEY_UP:
				vw.selIdx = max<size_t>(0, vw.selIdx - 1);
				break;
			case KEY_DOWN:
				// size() is 1 higher than max to allow selecting pin at the end
				vw.selIdx = min(vw.size(), vw.selIdx + 1);
				break;
			case KEY_ENTER:
			case 10 /* RETURN */:
