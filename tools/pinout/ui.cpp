...
void drawPinSet(Window &, int &row, const vector<string> &pkgs, unordered_map<string, string> &signals,
		const db::Signalset &pv);
...
void editPinSet(Window &win, int &row, const vector<Package> &pkgs, unordered_map<string, string> &signals, PinView &pv) {
	...
	int sgnCol = col + 1;
	PinsetEventer pev(formWin, form, pkgs, signals, pv.signalset.signals, sgnCol);
...
void drawPinSetEditingWindow(Window &win, PinSetView &vw) {
	...
	for ( vector<PinView>::iterator it = vw.pinViews.begin();
		...
		if ( idx == vw.selIdx ) {
			...
			scrollToAccomodate(win, pv.signalset.signals.size() + 1 /* header */ + 1 /* horiz. ruler */, row);
			...
			if ( row + pv.signalset.signals.size() + 2 < win.maxRows() ) { // TODO: ugly to check this here?
				drawPinSet(win, row, vw.pkgs, vw.signalDescs, pv);
				mvwhline(win, row++, 1, 0, min(win.maxCols() - 2, 79)); // capped at 80, esthaetics
			} else {
...
void drawTopWindow(BorderedWindow &win, const Datasheet &ds, DatabaseTotals &ttl, DatabaseTotals &sprt) {
	int topLine = 0;
	int topCol = 1;
	win.setTitle("Search");
