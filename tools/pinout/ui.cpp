...
class PinsetEventer : public FormEventHandler {
  public:
	PinsetEventer(Window &win, EventEmittingForm<PinsetEventer> &form, const vector<string> &pkgs, unordered_map<string, string> &signals,
				  int sgnCol) : window(win), form(form), packages(pkgs), signalAndDescMap(signals), signalColumn(sgnCol) {
...
void drawPinSetEditingWindow(Window &win, PinSetView &vw) {
	...
	for ( PinView &pv : vw.pinViews ) {
		...
		mvwhline(win.raw(), row++, 1, 0, horizontalLineLength);
...
void reorderPackages(vector<string> &pkgs) {
	...
	do {
		...
		center.paint();
	} while ( (pressedKey = wgetch(center.raw())) != KEY_ENTER && pressedKey != 10 );
}
