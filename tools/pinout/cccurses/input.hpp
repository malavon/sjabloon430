#ifndef CCURSES_INPUT_HPP
#define CCURSES_INPUT_HPP

#include "window.hpp"

namespace cccurses {

enum KeyFeedback { FEEDBACK_CONTINUE, FEEDBACK_STOP };

template<class CTX>
class KeyEventDelegate {
  public:
	static KeyFeedback keyBackTab(CTX &context) {
		return FEEDBACK_CONTINUE;
	}

	static KeyFeedback keyBackspace(CTX &context) {
		return FEEDBACK_CONTINUE;
	}

	// TODO: char or int?
	static KeyFeedback keyCharacter(CTX &context, const char ch) {
		return FEEDBACK_CONTINUE;
	}

	static KeyFeedback keyDelete(CTX &context) {
		return FEEDBACK_CONTINUE;
	}

	static KeyFeedback keyDownArrow(CTX &context) {
		return FEEDBACK_CONTINUE;
	}

	static KeyFeedback keyEnd(CTX &context) {
		return FEEDBACK_CONTINUE;
	}

	static KeyFeedback keyEnter(CTX &context) {
		return FEEDBACK_CONTINUE;
	}

	static KeyFeedback keyEscape(CTX &context) {
		return FEEDBACK_CONTINUE;
	}

	static KeyFeedback keyFunctionN(CTX &context, int n) {
		return FEEDBACK_CONTINUE;
	}

	static KeyFeedback keyHome(CTX &context) {
		return FEEDBACK_CONTINUE;
	}

	static KeyFeedback keyLeftArrow(CTX &context) {
		return FEEDBACK_CONTINUE;
	}

	static KeyFeedback keyRightArrow(CTX &context) {
		return FEEDBACK_CONTINUE;
	}

	static KeyFeedback keyTab(CTX &context) {
		return FEEDBACK_CONTINUE;
	}

	static KeyFeedback keyUpArrow(CTX &context) {
		return FEEDBACK_CONTINUE;
	}

	// TODO: char or int?
	static KeyFeedback keyWhitespace(CTX &context, const char ch) {
		return FEEDBACK_CONTINUE;
	}
};

template<class DELEGATE, class CTX>
class KeyEventProducer {
  public:
	static void captureAndDelegate(const Window &window, CTX &ctx) {
		int ch;
		KeyFeedback fb = FEEDBACK_CONTINUE;
		// check feedback first, otherwise won't be leaving form until pressed twice
		while ( fb == FEEDBACK_CONTINUE && (ch = wgetch(window.raw())) ) {
			switch ( ch ) {
				// TODO: TAB doesn't work, BTAB (back-tab does)...
				// using code 9 does too, no key value? odd
				case 9:
					fb = DELEGATE::keyTab(ctx);
					break;
				case KEY_BTAB:
					fb = DELEGATE::keyBackTab(ctx);
					break;
				case KEY_ENTER:
				case 10:
					fb = DELEGATE::keyEnter(ctx);
					break;
				case 27 /* escape */:
					fb = DELEGATE::keyEscape(ctx);
					break;
				case KEY_DOWN:
					fb = DELEGATE::keyDownArrow(ctx);
					break;
				case KEY_UP:
					fb = DELEGATE::keyUpArrow(ctx);
					break;
				case KEY_LEFT:
					fb = DELEGATE::keyLeftArrow(ctx);
					break;
				case KEY_RIGHT:
					fb = DELEGATE::keyRightArrow(ctx);
					break;
				case KEY_BACKSPACE:
				case 127: // 127 = 0177; returned for backspace in Konsole? why?
					fb = DELEGATE::keyBackspace(ctx);
					break;
					// if no navigation (left/right), delete still needed for last character in field!
				case KEY_DC: // delete character
					fb = DELEGATE::keyDelete(ctx);
					break;
				case KEY_END:
					fb = DELEGATE::keyEnd(ctx);
					break;
				case KEY_HOME:
					fb = DELEGATE::keyHome(ctx);
					break;
				case ' ': // do not allow space in some forms
					fb = DELEGATE::keyWhitespace(ctx, ch);
					break;
				case KEY_F(1):
				case KEY_F(2):
				case KEY_F(3):
				case KEY_F(4):
				case KEY_F(5):
				case KEY_F(6):
				case KEY_F(7):
				case KEY_F(8):
				case KEY_F(9):
				case KEY_F(10):
				case KEY_F(11):
				case KEY_F(12):
				case KEY_F(13):
					fb = DELEGATE::keyFunctionN(ctx, ch - KEY_F0);
					break;
				default:
					fb = DELEGATE::keyCharacter(ctx, ch);
					break;
			}
		}
	}
};
} // namespace cccurses
#endif // CCURSES_INPUT_HPP
