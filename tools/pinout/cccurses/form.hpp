...
template<class EH>
class EventingFormKeyEventConsumer : public KeyEventConsumer<FormEventHandlerWrapper<EH>> /*, private SimpleFormKeyEventConsumer*/ {
  public:
...
class EventEmittingForm : public Form<EventingFormKeyEventConsumer<EH>> {
  public:
	...
	void loop(EH &callback) {
		...
		KeyEventProducer<EventingFormKeyEventConsumer<EH>, FormEventHandlerWrapper<EH> &>::captureAndConsume(localWin, wrapper);
	}
};
...
class SimpleForm : public Form<SimpleFormKeyEventConsumer> {
  public:
	...
	SimpleForm(const Window &win, const Window &formSub, vector<Field> fields) : Form<SimpleFormKeyEventConsumer>(win, formSub, fields) { }
